...
import { IconSearch, IconChevronDown, IconUserCheck, IconPencil, IconTrash } from '@/components/icons'
...
const SORT_COLUMNS = [{ key: 'sr_no', label: 'Sr. No.' }]
...
export default function ItAssetHoldersPage() {
  ...
  const [itAssetSubtype, setItAssetSubtype] = useState('')
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (ownCompany) params.set('own_company', ownCompany)
  ...
  async function handleDeleteClick(row) {
    setDeleteTarget(row)
    const res = await apiFetch(`/api/it-asset/it-asset-holders/${row.it_asset_holder_id}/check-delete/`)
    setDeleteInfo(await res.json())
  }

  async function confirmDelete() {
    const res = await apiFetch(`/api/it-asset/it-asset-holders/${deleteTarget.it_asset_holder_id}/`, {
      method: 'DELETE',
    })
    setDeleteInfo(null)
    if (res.status === 204) {
      toast.success('Deleted')
      setRows((prev) => prev.filter((r) => r.it_asset_holder_id !== deleteTarget.it_asset_holder_id))
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card">
        <div className="overflow-x-auto">
          <table className="w-full table-fixed border-collapse text-sm">
            ...
            <tbody>
              {rows.map((r, idx) => {
                const open = openRow === r.it_asset_holder_id
                ...
                return (
                  <Fragment key={r.it_asset_holder_id}>
                    <tr
                      ...
                      onClick={() => setOpenRow(open ? null : r.it_asset_holder_id)}
                    >
                      ...
                      <td className="truncate px-3 py-2.5" title={r.it_asset_subtype_name}>
                        {r.it_asset_subtype_name || '—'}
                      </td>
                      <td className="truncate px-3 py-2.5" title={r.holder_company_name}>
                        {r.holder_company_abrv || r.holder_company_name || '—'}
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {canEdit && (
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation()
                                navigate(`/it-asset/it-asset-holders/${r.it_asset_holder_id}/edit`)
                              }}
                      ...
                      <tr className={`${zebra} border-b border-border/60`}>
                        <td colSpan={9} className="px-5 py-4">
                          <div className="mb-3 flex items-center gap-2">
                            {canEdit && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => navigate(`/it-asset/it-asset-holders/${r.it_asset_holder_id}/edit`)}
                              >
                                <IconPencil className="h-3.5 w-3.5" />
                                Edit
                              </Button>
                          ...
                          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            <DetailField label="Asset Tag" value={r.it_asset_tag} />
                            <DetailField label="SAP Code" value={r.it_asset_sap_code} />
                            <DetailField label="Manufacturer" value={r.it_asset_mfg_name} />
                            <DetailField label="Own Company" value={r.own_company_name} />
                            <DetailField label="Holder Company" value={r.holder_company_name} />
                            <DetailField label="Department" value={r.department_name} />
                            <DetailField label="Location" value={r.company_loc_name} />
                            <DetailField
                              label="Asset Active"
                              value={r.it_asset_active === 'N' ? 'Inactive' : 'Active'}
                            />
                            <DetailField label="Remark" value={r.holder_remark} wide />
                          </div>
                        </td>
                      </tr>
                    )}
