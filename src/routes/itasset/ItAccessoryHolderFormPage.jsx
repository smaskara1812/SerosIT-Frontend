...
export default function ItAccessoryHolderFormPage() {
  const { id } = useParams()
  ...
  const heading = isEdit ? `Holder record — ${form.it_accessory_name || ''}` : 'New IT Accessory Holder'
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isEdit) {
        navigate(`/it-asset/it-accessory-holders/${data.it_accessory_holder_id}/edit`, { replace: true })
      } else {
          ...
          <div key={section.title} className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {section.fields.map((f) => (
                <div key={f.name} className={`flex flex-col gap-1.5 ${f.wide ? 'col-span-full' : ''}`}>
                  <Label>
                    {f.label}
